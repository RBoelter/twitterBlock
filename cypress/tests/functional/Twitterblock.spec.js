describe('Twitter Block plugin tests', function () {
		...
		cy.get('input[id^="select-cell-twitterblockplugin-enabled"]')
			.then($btn => {
				if ($btn.attr('checked') === 'checked') {
					...
					cy.get('div:contains(\'The plugin "Twitter Block" has been disabled.\')');
				}
			});
	});
	...
	it('Enable Twitter Block', function () {
		...
		cy.waitJQuery();
		cy.get('tr[id="component-grid-settings-plugins-settingsplugingrid-category-blocks-row-twitterblockplugin"] a[class="show_extras"]').click();
		cy.get('a[id^="component-grid-settings-plugins-settingsplugingrid-category-blocks-row-twitterblockplugin-settings-button"]').click();
		// Fill out settings form
		cy.get('form[id="twitterSettings"] input[name="tweetTitle"]')
			.clear()
			.type('Tweets by <a href="https://twitter.com/JohnWillinsky" target="_blank">@JohnWillinsky</a>');
		...
		cy.get('div[class*="pkpTabs--side"] #setup input[value="twitterblockplugin"]')
			.then($btn => {
				if ($btn.attr('checked') !== 'checked' && $btn.attr('checked') !== true) {
					cy.get('div[class*="pkpTabs--side"] #setup input[value="twitterblockplugin"]').check();
